...
Amp\Loop::run(function () use ($service, $logger) {
    ...
    // Handler for chat2desc events
    $router->addRoute('POST', '/chat2desc-incoming', new CallableRequestHandler(function (Request $request) use ($service, $logger) {
        $buffer = '';
        while (($chunk = yield $request->getBody()->read()) !== null) {
            $buffer .= $chunk;
        }
        try {
            $service->handleChat2DeskEvent($buffer);
        } catch (Exception $exception) {
            $logger->error('Failed to handle chat2desc event', ['message' => $exception->getMessage(), 'code' => $exception->getCode()]);
            throw $exception;
        }
